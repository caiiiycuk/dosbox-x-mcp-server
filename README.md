...
Minimal MCP server for controlling the DOSBox-X built-in debugger.

The server exposes a small set of MCP tools and forwards debugger work to
DOSBox-X over a local text protocol. The primary primitive is `debug_exec(command)`,
which runs an existing DOSBox-X debugger command and returns captured debugger
output.
...
stderr. DOSBox-X connects separately over TCP on `127.0.0.1:58991`.
